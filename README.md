...
| 사용자 | 사용자 니즈 |
| --- | --- |
| **팀 프로젝트 조장** | 공지사항 전달, 역할 분담, 이슈 공유, 파일 버전 관리 |
| **팀 프로젝트 조원** | 공지사항 확인, 역할 확인, 이슈 공유, 파일 버전 관리 |
...
| 사용자 | 페르소나 |
| --- | --- |
| **팀 프로젝트 조장** | **이름**: 김머랭<br>**나이**: 24세<br>**전공**: 경영학<br>**역할**: 명지대학교 경영학 수업의 팀 프로젝트 조장<br>**목표**: 팀을 보다 효율적으로 이끌고 싶다.<br>**니즈**: 공지사항 관리, 역할 분담 관리, 이슈 공유, 파일 버전 관리. |
| **팀 프로젝트 조원** | **이름**: 송모코<br>**나이**: 20세<br>**전공**: 경영학<br>**역할**: 명지대학교 경영학 수업의 팀 프로젝트 조원<br>**목표**: 팀 프로젝트에 적극적으로 참여하고 싶다.<br>**니즈**: 공지사항 확인, 역할 분담 결과 확인, 이슈 공유, 파일 버전 관리. |
...
- 시나리오:
    ...
    - 김머랭은 업데이트된 ppt 파일을 업로드합니다. 이는 최신 버전으로 관리됩니다.
...
- 시나리오:
    ...
    - 송모코는 업데이트된 자료조사 파일을 업로드합니다. 이는 최신 버전으로 관리됩니다.
...
| 페르소나 | 시나리오 | 요구사항 |
| --- | --- | --- |
| **팀 프로젝트 조장** |  조장이 된 김머랭은 스마트폰에서 서비스를 실행합니다.<br> 서비스의 메인 화면에서 팀 만들기를 선택한 김머랭은 조 정보(이름, 인원 수 등)을 입력하여 팀을 생성합니다.<br> 김머랭은 팀을 생성해 만들어진 QR 코드를 조원들에게 공유합니다.<br> 김머랭은 회의를 통해 분담한 역할을 서비스에 등록합니다.<br> 김머랭은 교수님에게 전달받은 사항을 공지사항으로 작성합니다.<br> 김머랭은 조원의 자료조사 중 잘못된 부분을 찾아 이슈로 등록합니다.<br> 김머랭은 업데이트된 ppt 파일을 업로드합니다. 이는 최신 버전으로 관리됩니다. | 1. 팀 생성 기능<br>2. 조원 별 역할 관리(CRUD) 기능<br>3. 공지사항 관리(CRUD) 기능<br>4. 이슈 관리(CRUD) 기능<br>5. 파일 업로드 기능<br>6. 파일 버전 관리 기능
| **팀 프로젝트 조원** |  조원이 된 송모코는 김머랭이 공유한 QR 을 스캔해 서비스에 접속합니다.<br> 서비스에 접속한 송모코는 이름을 입력해 팀에 합류합니다.<br> 송모코는 회의를 통해 정해진 역할을 서비스를 통해 확인합니다.<br> 송모코는 김머랭이 등록한 공지사항을 지속적으로 조회하고 인지합니다.<br> 송모코는 자료조사 중 도움이 필요해 이슈를 등록합니다.<br> 송모코는 업데이트된 자료조사 파일을 업로드합니다. 이는 최신 버전으로 관리됩니다. | 1. 팀 참여 기능<br>2. 조원 별 역할 조회 기능<br>3. 공지사항 조회 기능<br>4. 이슈 관리(CRUD) 기능<br>5. 파일 업로드 기능<br>6. 파일 버전 관리 기능 |
...
### 팀 프로젝트 조장 - 파일 버전 관리 기능

- 요구사항: 파일 버전 관리 기능
- 사용자 스토리: “조장으로써, 파일 버전을 효과적으로 관리하기 위해 파일 버전 관리 기능을 이용하고 싶습니다.”
- 인수 조건:
    - Given: 조장은 PPT 파일을 수정하고, 이를 업로드하기 위해 서비스에 접속합니다.
    - When: 조장이 파일 업로드를 선택합니다.
    - Then: 서비스는 업로드한 파일을 “최신 파일” 로 만들고, 기존 파일을 “이전 파일” 목록에 추가합니다.
...
### 팀 프로젝트 조원 - 파일 버전 관리 기능

- 요구사항: 파일 버전 관리 기능
- 사용자 스토리: “조원으로써, 파일 버전을 효과적으로 관리하기 위해 파일 버전 관리 기능을 이용하고 싶습니다.”
- 인수 조건:
    - Given: 조원은 자료조사 파일을 수정하고, 이를 업로드하기 위해 서비스에 접속합니다.
    - When: 조원이 파일 업로드를 선택합니다.
    - Then: 서비스는 업로드한 파일을 “최신 파일” 로 만들고, 기존 파일을 “이전 파일” 목록에 추가합니다.
